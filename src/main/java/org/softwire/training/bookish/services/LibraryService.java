...
public class LibraryService extends DatabaseService {
    ...
    public List<Member> getAllUsers() {
        List<Map<String,Object>> rows = jdbi.withHandle(handle ->
                handle.createQuery("SELECT * FROM bookish.members m LEFT OUTER JOIN bookish.copies c ON m.MemberID = c.MemberID LEFT JOIN bookish.books b ON c.BookID = b.BookID")
                        .mapToMap()
                        .list());
        ...
        for (Map<String,Object> row : rows) {
            ...
            Member member = memberMap.computeIfAbsent(memberID, id -> {
                ...
                newMember.setStatus((String) row.get("status"));
                return newMember;
            });
            ...
            member.book.getCopies().add(copy);
        }
